...
save_dir = "/home/11001207/chawanP/Teerapol/MM_retrieval_rq3/embedding_dir_rq1"
        ...
        if 'captions' in current_entry and isinstance(current_entry['captions'], dict):
            for lang_code, captions_list in current_entry['captions'].items():
                if isinstance(captions_list, list) and captions_list:
                    ...
                    # print(f"lang_text_embeddings: {lang_text_embeddings}")
                    # print(f'lang_text_embeddings shape: {lang_text_embeddings[0].shape}')
                    lang_text_embeddings = concatenate_tensors(lang_text_embeddings)
                    # print(f"lang_text_embeddings shape after concat: {lang_text_embeddings.shape}")
